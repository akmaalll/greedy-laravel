                    ...
                    <div class="card-body pt-4">
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <p class="h6"><i class="ri-map-pin-line me-1"></i> {{ $data->pesanan->lokasi_acara }}
                                </p>
                            </div>
                            <div class="col-md-6">
                                <h6 class="text-muted mb-1">Jadwal:</h6>
                                <p class="h6"><i class="ri-calendar-event-line me-1"></i>
                                    {{ $data->pesanan->tanggal_acara ? \Carbon\Carbon::parse($data->pesanan->tanggal_acara)->format('d M Y') : '-' }}
                                    ({{ $data->pesanan->waktu_mulai_acara ?? '-' }})</p>
                            </div>
                        </div>
                    ...
                    <div class="card-body pt-4">
                        <div class="d-flex align-items-center mb-3">
                            <div class="avatar avatar-sm me-2">
                                <span
                                    class="avatar-initial rounded-circle bg-label-dark">{{ substr($data->pesanan->klien->name, 0, 1) }}</span>
                            </div>
                            <div>
                                <h6 class="mb-0">{{ $data->pesanan->klien->name }}</h6>
                                <small class="text-muted d-block">{{ $data->pesanan->klien->email }}</small>
                                @if ($data->pesanan->klien->no_hp)
                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', str_replace('08', '628', $data->pesanan->klien->no_hp)) }}"
                                        target="_blank" class="text-muted small">
                                        <i
                                            class="ri-whatsapp-line me-1 text-success"></i>{{ $data->pesanan->klien->no_hp }}
                                    </a>
                                @else
